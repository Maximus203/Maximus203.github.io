import type { Language } from '@/types';

export const UI_LABELS: Record<Language, Record<string, string>> = {
  fr: {
    home: 'Accueil',
    gallery: 'Galerie',
    tools: 'Outils',
    about: 'À Propos',
    experience: 'Expériences',
    projects: 'Projets',
    skills: 'Compétences',
    education: 'Formation',
    available: 'Disponible pour nouveaux projets',
    role: 'Développeur',
    and: '&',
    mentor: 'Formateur',
    // Profile-specific roles for H1 (issue #22)
    roleSoftware: 'Développeur',
    roleNetwork: 'Expert Digitalisation',
    roleTeaching: 'Formateur',
    download: 'Télécharger',
    resume: 'CV',
    base: 'Base',
    status: 'Statut',
    impactMetricLabel: 'Gain process FIDECA',
    impactMetricTransition: 'vers',
    skip: 'Passer',
    introDialogLabel: "Animation d'introduction",
    top: 'Haut',
    selectedProjects: 'Projets sélectionnés',
    technicalSkills: 'Compétences techniques',
    workExperience: 'Expériences professionnelles',
    educationTitle: 'Formation académique',
    galleryTitle: 'Moments & événements',
    gallerySubtitle: 'Conférences, hackathons et vie communautaire.',
    toolsTitle: 'Boîte à outils',
    toolsSubtitle: 'Des utilitaires pratiques pour développeurs et créateurs.',
    toolReadme: 'Générateur readme',
    toolReadmeDesc: 'Créez le profil GitHub parfait avec des badges et stats dynamiques.',
    // Readme Generator specific (issue #28 onboarding)
    readmeStartUsername: 'Ton pseudo GitHub',
    readmeStartHere: 'commence ici',
    readmeStartHelp: "Saisis ton pseudo : l'aperçu et les statistiques deviennent les tiens instantanément.",
    toolMeme: 'Générateur de meme',
    toolMemeDesc: 'Créez du contenu viral rapidement pour vos réseaux sociaux.',
    toolConverter: 'Convertisseur d\'images',
    toolConverterDesc: 'Optimisez vos assets : convertissez 1 image ou un lot de 10 simultanément.',
    // Image Converter specific
    converterTitle: 'Convertisseur d\'Images',
    converterSubtitle: 'Convertissez vos images localement, sans envoi vers un serveur.',
    backToTools: 'Retour aux outils',
    qualityLabel: 'Qualité:',
    convertAll: 'Convertir Tout',
    retry: 'Réessayer',
    downloadAllZip: 'Tout télécharger (.zip)',
    dropImagesHere: 'Glissez vos images ici',
    supportedFormats: 'JPG, PNG, WEBP, GIF, SVG supportés',
    selectFiles: 'Sélectionner des fichiers',
    converterWeightSaved: 'Poids économisé',
    converterWeightIncreased: 'Poids augmenté',
    converterPercentageSaved: 'économisé',
    converterPercentageIncreased: 'augmenté',
    converterSavedBadge: '−{pct}% économisé',
    converterIncreasedBadge: '+{pct}% augmenté',
    converterBefore: 'Avant',
    converterAfter: 'Après',
    converterWeightReduced: 'de poids',
    converterTryDemo: 'Essayer avec une image de démo',
    converterDropImages: 'Déposez vos images ci-dessus : la conversion est automatique et vous voyez aussitôt le poids économisé.',
    converterFailed: 'Échec',
    converterPending: 'En attente',
    converterUnexpectedError: 'Erreur inattendue lors du traitement.',
    converterCorruptedFile: 'Fichier image corrompu ou illisible.',
    converterArchiveError: "Une erreur est survenue lors de la création de l'archive.",
    converterDemoLoadError: "Impossible de charger l'image de démo. Réessayez ou déposez votre propre image.",
    launch: 'Lancer',
    searchPlaceholder: 'Rechercher un projet...',
    startProject: 'Proposer un projet',
    submitProjectTitle: 'Parlons de votre projet',
    submitProjectDesc: 'Remplissez le formulaire ci-dessous et je vous répondrai rapidement.',
    firstName: 'Prénom',
    lastName: 'Nom',
    email: 'Email',
    projectDetails: 'Détails du projet',
    attachFile: 'Joindre un fichier',
    filePlaceholder: 'Glissez un fichier ou cliquez',
    send: 'Envoyer la demande',
    cancel: 'Annuler',
    successMessage: 'Votre demande a été envoyée avec succès !',
    memeTopText: 'Texte du Haut',
    memeBottomText: 'Texte du Bas',
    memeUpload: 'Importer une image',
    memeDownload: 'Télécharger le Meme',
    memeFontSize: 'Taille du texte',
    memeTextColor: 'Couleur du texte',
    memeStrokeColor: 'Couleur du contour',
    memeDragDrop: 'Glissez une image ici',
    memeExampleTop: 'QUAND LE CODE MARCHE',
    memeExampleBottom: 'DU PREMIER COUP',
    // Footer
    footerQuote: 'Développeur Full-stack créant des expériences web modernes.',
    sections: 'Sections',
    pages: 'Pages & outils',
    connect: 'Me suivre',
    builtWith: 'Construit avec',
    hostedOn: 'Hébergé sur',
    rights: 'Tous droits réservés.',
    allProjects: 'Tous les projets',
    // Students
    students: 'Étudiants',
    studentsTitle: 'Réalisations de mes étudiants',
    studentsSubtitle: 'Portfolios réalisés par mes étudiants durant leur formation.',
    studentsCount: 'portfolios mis en avant',
    studentsHighlight: 'Créés pendant leur parcours de formation',
    // SEO
    seoHomeTitle: "Cherif Diouf | Développeur Full-Stack, Digitalisation & Formateur – Dakar",
    seoHomeDesc: "Portfolio de El Hadji Ahmadou Cherif Diouf. Expert Laravel, React, Supabase & DevOps. Développement web/mobile, digitalisation d'entreprise, prompt engineering et formation au Sénégal.",
    seoGalleryTitle: "Galerie & Événements Tech | Cherif Diouf – Dakar, Sénégal",
    seoGalleryDesc: "Photos et moments des conférences, hackathons, graduations et événements tech au Sénégal. Cherif Diouf, développeur full-stack et formateur à Dakar.",
    seoToolsTitle: "Outils Gratuits pour Développeurs | Cherif Diouf",
    seoToolsDesc: "Outils open-source gratuits : Générateur de README GitHub, Convertisseur WebP, Créateur de Memes. Par Cherif Diouf, développeur full-stack au Sénégal.",
    seoStudentsTitle: "Réalisations de mes Étudiants | Cherif Diouf Formateur",
    seoStudentsDesc: "Portfolios web réalisés par les étudiants de Cherif Diouf durant leur formation en développement web à Dakar (ESTM, ISM, ESCOA).",
    // Profile switcher
    profileSoftware: 'Génie Logiciel',
    profileNetwork: 'Admin Réseaux',
    profileTeaching: 'Enseignement',
    // Hero entry CTAs (issue #22)
    heroCtaDev: 'Je cherche un développeur',
    heroCtaDigitalization: 'Je cherche un expert digitalisation',
    heroCtaTrainer: 'Je cherche un formateur',
    viewProjects: 'Explorer mes réalisations',
    contactCta: 'Discutons ensemble',
    discussProject: 'Discutons de votre projet',
    publications: 'Publications',
    // Profile-specific bios (issue #22)
    bioShortSoftware: 'Développeur Full-Stack. Je conçois des applications web modernes, performantes et évolutives (React, Laravel, Supabase, DevOps).',
    bioShortNetwork: 'Expert digitalisation & Admin Systèmes. J\'optimise infrastructures, réseaux et processus métier pour gains de temps massifs.',
    bioShortTeaching: 'Formateur développeur. J\'accompagne étudiants et équipes vers l\'autonomie technique par la pratique projet et la pédagogie.',
  },
  en: {
    home: 'Home',
    gallery: 'Gallery',
    tools: 'Tools',
    about: 'About',
    experience: 'Experience',
    projects: 'Projects',
    skills: 'Skills',
    education: 'Education',
    available: 'Available for new projects',
    role: 'Developer',
    and: '&',
    mentor: 'Mentor',
    // Profile-specific roles for H1 (issue #22)
    roleSoftware: 'Developer',
    roleNetwork: 'Digitalization Expert',
    roleTeaching: 'Trainer',
    download: 'Download',
    resume: 'Resume',
    base: 'Base',
    status: 'Status',
    impactMetricLabel: 'Process gain FIDECA',
    impactMetricTransition: 'to',
    skip: 'Skip',
    introDialogLabel: 'Introduction animation',
    top: 'Top',
    selectedProjects: 'Selected projects',
    technicalSkills: 'Technical skills',
    workExperience: 'Work experience',
    educationTitle: 'Education',
    galleryTitle: 'Moments & events',
    gallerySubtitle: 'Conferences, hackathons and community life.',
    toolsTitle: 'Dev toolbox',
    toolsSubtitle: 'Useful utilities for developers and creators.',
    toolReadme: 'Readme generator',
    toolReadmeDesc: 'Create the perfect GitHub profile with dynamic badges and stats.',
    // Readme Generator specific (issue #28 onboarding)
    readmeStartUsername: 'Your GitHub username',
    readmeStartHere: 'start here',
    readmeStartHelp: 'Enter your username: the preview and stats become yours instantly.',
    toolMeme: 'Meme generator',
    toolMemeDesc: 'Create viral content quickly for your social media.',
    toolConverter: 'Image converter',
    toolConverterDesc: 'Optimize assets: convert 1 image or a batch of 10 simultaneously.',
    // Image Converter specific
    converterTitle: 'Image Converter',
    converterSubtitle: 'Convert your images locally, without uploading to a server.',
    backToTools: 'Back to Tools',
    qualityLabel: 'Quality:',
    convertAll: 'Convert All',
    retry: 'Retry',
    downloadAllZip: 'Download All (.zip)',
    dropImagesHere: 'Drop your images here',
    supportedFormats: 'JPG, PNG, WEBP, GIF, SVG supported',
    selectFiles: 'Select Files',
    converterWeightSaved: 'Weight saved',
    converterWeightIncreased: 'Weight increased',
    converterPercentageSaved: 'saved',
    converterPercentageIncreased: 'increased',
    converterSavedBadge: '−{pct}% saved',
    converterIncreasedBadge: '+{pct}% increased',
    converterBefore: 'Before',
    converterAfter: 'After',
    converterWeightReduced: 'weight reduced',
    converterTryDemo: 'Try with a demo image',
    converterDropImages: 'Drop your images above: conversion is automatic and you immediately see the weight saved.',
    converterFailed: 'Failed',
    converterPending: 'Pending',
    converterUnexpectedError: 'Unexpected error while processing.',
    converterCorruptedFile: 'Corrupted or unreadable image file.',
    converterArchiveError: 'An error occurred while creating the archive.',
    converterDemoLoadError: 'Could not load the demo image. Try again or drop your own image.',
    launch: 'Launch',
    searchPlaceholder: 'Search projects...',
    startProject: 'Propose a project',
    submitProjectTitle: "Let's discuss your project",
    submitProjectDesc: 'Fill out the form below and I will get back to you shortly.',
    firstName: 'First Name',
    lastName: 'Last Name',
    email: 'Email',
    projectDetails: 'Project Details',
    attachFile: 'Attach File',
    filePlaceholder: 'Drag a file or click to upload',
    send: 'Send Request',
    cancel: 'Cancel',
    successMessage: 'Your request has been sent successfully!',
    memeTopText: 'Top Text',
    memeBottomText: 'Bottom Text',
    memeUpload: 'Upload Image',
    memeDownload: 'Download Meme',
    memeFontSize: 'Font Size',
    memeTextColor: 'Text Color',
    memeStrokeColor: 'Outline Color',
    memeDragDrop: 'Drag & Drop an image here',
    memeExampleTop: 'WHEN THE CODE WORKS',
    memeExampleBottom: 'ON THE FIRST TRY',
    // Footer
    footerQuote: 'Full-stack developer crafting modern web experiences.',
    sections: 'Sections',
    pages: 'Pages & tools',
    connect: 'Connect',
    builtWith: 'Built with',
    hostedOn: 'Hosted on',
    rights: 'All rights reserved.',
    allProjects: 'All projects',
    // Students
    students: 'Students',
    studentsTitle: 'Student achievements',
    studentsSubtitle: 'Portfolios built by my students during their training.',
    studentsCount: 'featured portfolios',
    studentsHighlight: 'Built during their training journey',
    // SEO
    seoHomeTitle: "Cherif Diouf | Full-Stack Developer, Digitalization Expert & Trainer – Dakar",
    seoHomeDesc: "Portfolio of El Hadji Ahmadou Cherif Diouf. Laravel, React, Supabase & DevOps Expert. Web/mobile development, business digitalization, prompt engineering and training in Senegal.",
    seoGalleryTitle: "Tech Events & Gallery | Cherif Diouf – Dakar, Senegal",
    seoGalleryDesc: "Photos from conferences, hackathons, graduations and tech events in Senegal. Cherif Diouf, full-stack developer and trainer based in Dakar.",
    seoToolsTitle: "Free Developer Tools | Cherif Diouf",
    seoToolsDesc: "Free open-source tools: GitHub README Generator, WebP Image Converter, Meme Creator. By Cherif Diouf, full-stack developer in Senegal.",
    seoStudentsTitle: "My Students' Achievements | Cherif Diouf – Web Trainer",
    seoStudentsDesc: "Web portfolios built by Cherif Diouf's students during their web development training in Dakar (ESTM, ISM, ESCOA).",
    // Profile switcher
    profileSoftware: 'Software Engineering',
    profileNetwork: 'Network Admin',
    profileTeaching: 'Teaching',
    // Hero entry CTAs (issue #22)
    heroCtaDev: 'I need a developer',
    heroCtaDigitalization: 'I need a digitalization expert',
    heroCtaTrainer: 'I need a trainer',
    viewProjects: 'Explore my work',
    contactCta: "Let's talk",
    discussProject: "Let's discuss your project",
    publications: 'Publications',
    // Profile-specific bios (issue #22)
    bioShortSoftware: 'Full-Stack Developer. I build modern, performant, scalable web applications (React, Laravel, Supabase, DevOps).',
    bioShortNetwork: 'Digitalization Expert & SysAdmin. I optimize infrastructure, networks and business processes for massive time savings.',
    bioShortTeaching: 'Developer Trainer. I guide students and teams to technical autonomy through project-based practice and pedagogy.',
  },
  zh: {
    home: '首页',
    gallery: '画廊',
    tools: '工具',
    about: '关于',
    experience: '经历',
    projects: '项目',
    skills: '技能',
    education: '教育',
    available: '可接受新项目',
    role: '开发者',
    and: '&',
    mentor: '导师',
    // Profile-specific roles for H1 (issue #22)
    roleSoftware: '开发者',
    roleNetwork: '数字化专家',
    roleTeaching: '讲师',
    download: '下载',
    resume: '简历',
    base: '常驻',
    status: '状态',
    impactMetricLabel: 'FIDECA 流程提速',
    impactMetricTransition: '到',
    skip: '跳过',
    introDialogLabel: '开场动画',
    top: '顶部',
    selectedProjects: '精选项目',
    technicalSkills: '技术技能',
    workExperience: '工作经历',
    educationTitle: '教育背景',
    galleryTitle: '时刻 & 活动',
    gallerySubtitle: '会议、黑客马拉松和社区生活。',
    toolsTitle: '开发工具箱',
    toolsSubtitle: '为开发者和创作者提供的实用工具。',
    toolReadme: 'Readme 生成器',
    toolReadmeDesc: '使用动态徽章和统计数据创建完美的 GitHub 个人资料。',
    // Readme Generator specific (issue #28 onboarding)
    readmeStartUsername: '你的 GitHub 用户名',
    readmeStartHere: '从这里开始',
    readmeStartHelp: '输入你的用户名：预览和统计将立即变成你的。',
    toolMeme: 'Meme 生成器',
    toolMemeDesc: '为您的社交媒体快速创建病毒式内容。',
    toolConverter: '图像转换器',
    toolConverterDesc: '优化资源：一次转换 1 张图像或批量处理 10 张。',
    // Image Converter specific
    converterTitle: '图像转换器',
    converterSubtitle: '在本地转换图像，无需上传到服务器。',
    backToTools: '返回工具',
    qualityLabel: '质量：',
    convertAll: '全部转换',
    retry: '重试',
    downloadAllZip: '全部下载 (.zip)',
    dropImagesHere: '将图片拖放到这里',
    supportedFormats: '支持 JPG、PNG、WEBP、GIF、SVG',
    selectFiles: '选择文件',
    converterWeightSaved: '节省的大小',
    converterWeightIncreased: '增加的大小',
    converterPercentageSaved: '节省',
    converterPercentageIncreased: '增加',
    converterSavedBadge: '节省 {pct}%',
    converterIncreasedBadge: '增加 {pct}%',
    converterBefore: '转换前',
    converterAfter: '转换后',
    converterWeightReduced: '的大小',
    converterTryDemo: '试用演示图片',
    converterDropImages: '将图片拖放到上方：转换自动进行，您可立即看到节省的大小。',
    converterFailed: '失败',
    converterPending: '等待中',
    converterUnexpectedError: '处理时发生意外错误。',
    converterCorruptedFile: '图片文件已损坏或无法读取。',
    converterArchiveError: '创建压缩包时发生错误。',
    converterDemoLoadError: '无法加载演示图片，请重试或拖放您自己的图片。',
    launch: '启动',
    searchPlaceholder: '搜索项目...',
    startProject: '提交项目需求',
    submitProjectTitle: '讨论您的项目',
    submitProjectDesc: '请填写下面的表格，我会尽快回复您。',
    firstName: '名字',
    lastName: '姓氏',
    email: '电子邮件',
    projectDetails: '项目详情',
    attachFile: '附件',
    filePlaceholder: '拖动文件或点击上传',
    send: '发送请求',
    cancel: '取消',
    successMessage: '您的请求已成功发送！',
    memeTopText: '顶部文字',
    memeBottomText: '底部文字',
    memeUpload: '上传图片',
    memeDownload: '下载 Meme',
    memeFontSize: '字体大小',
    memeTextColor: '文字颜色',
    memeStrokeColor: '描边颜色',
    memeDragDrop: '将图片拖放到此处',
    memeExampleTop: '当代码运行成功',
    memeExampleBottom: '而且是第一次',
    // Footer
    footerQuote: '打造现代网络体验的全栈开发人员。',
    sections: '章节',
    pages: '页面与工具',
    connect: '连接',
    builtWith: '构建于',
    hostedOn: '托管于',
    rights: '版权所有。',
    allProjects: '所有项目',
    // Students
    students: '学生',
    studentsTitle: '学生作品',
    studentsSubtitle: '我的学生在培训期间制作的作品集。',
    studentsCount: '个精选作品集',
    studentsHighlight: '在培训过程中创建',
    // SEO
    seoHomeTitle: "Cherif Diouf | 全栈开发者、数字化专家 & 讲师 – 达喀尔",
    seoHomeDesc: "El Hadji Ahmadou Cherif Diouf 的作品集。Laravel、React、Supabase & DevOps 专家。塞内加尔网页/移动应用开发、企业数字化转型与提示工程。",
    seoGalleryTitle: "技术活动图库 | Cherif Diouf – 达喀尔，塞内加尔",
    seoGalleryDesc: "塞内加尔技术会议、黑客马拉松和毕业典礼的精彩瞬间。全栈开发者 Cherif Diouf。",
    seoToolsTitle: "免费开发者工具 | Cherif Diouf",
    seoToolsDesc: "免费开源工具：GitHub README 生成器、WebP 图片转换器、Meme 创建器。由塞内加尔全栈开发者 Cherif Diouf 提供。",
    seoStudentsTitle: "学生作品展示 | Cherif Diouf 讲师",
    seoStudentsDesc: "Cherif Diouf 在达喀尔（ESTM、ISM、ESCOA）教授Web开发期间学生制作的作品集。",
    // Profile switcher
    profileSoftware: '软件工程',
    profileNetwork: '网络管理',
    profileTeaching: '教学',
    // Hero entry CTAs (issue #22)
    heroCtaDev: '我要找开发者',
    heroCtaDigitalization: '我要找数字化专家',
    heroCtaTrainer: '我要找讲师',
    viewProjects: '探索我的项目',
    contactCta: '一起合作',
    discussProject: '聊聊您的项目',
    publications: '发表论文',
    // Profile-specific bios (issue #22)
    bioShortSoftware: '全栈开发者。构建现代、高性能、可扩展的Web应用 (React, Laravel, Supabase, DevOps)。',
    bioShortNetwork: '数字化专家 & 系统管理员。优化基础设施、网络和业务流程，实现大幅时间节省。',
    bioShortTeaching: '开发讲师。通过项目实战和教学法，引导学生和团队走向技术自主。',
  },
  ja: {
    home: 'ホーム',
    gallery: 'ギャラリー',
    tools: 'ツール',
    about: '私について',
    experience: '経歴',
    projects: 'プロジェクト',
    skills: 'スキル',
    education: '学歴',
    available: '新規案件募集中',
    role: '開発者',
    and: '&',
    mentor: '講師',
    // Profile-specific roles for H1 (issue #22)
    roleSoftware: '開発者',
    roleNetwork: 'デジタル化専門家',
    roleTeaching: '講師',
    download: 'ダウンロード',
    resume: '履歴書',
    base: '拠点',
    status: 'ステータス',
    impactMetricLabel: 'FIDECA 業務短縮',
    impactMetricTransition: 'から',
    skip: 'スキップ',
    introDialogLabel: 'イントロアニメーション',
    top: 'トップ',
    selectedProjects: '主なプロジェクト',
    technicalSkills: '技術スキル',
    workExperience: '職務経歴',
    educationTitle: '学歴',
    galleryTitle: '瞬間 & イベント',
    gallerySubtitle: 'カンファレンス、ハッカソン、コミュニティライフ。',
    toolsTitle: '開発ツールボックス',
    toolsSubtitle: '開発者とクリエイターのための便利なユーティリティ。',
    toolReadme: 'Readme ジェネレーター',
    toolReadmeDesc: '動的なバッジと統計を使用して完璧な GitHub プロファイルを作成します。',
    // Readme Generator specific (issue #28 onboarding)
    readmeStartUsername: 'あなたの GitHub ユーザー名',
    readmeStartHere: 'ここから始める',
    readmeStartHelp: 'ユーザー名を入力：プレビューと統計が即座にあなたのものになります。',
    toolMeme: 'ミームジェネレーター',
    toolMemeDesc: 'ソーシャルメディア向けのバイラルコンテンツをすばやく作成します。',
    toolConverter: '画像コンバーター',
    toolConverterDesc: 'アセットの最適化：1枚の画像または10枚のバッチを同時に変換します。',
    // Image Converter specific
    converterTitle: '画像コンバーター',
    converterSubtitle: '画像をローカルで変換し、サーバーにアップロードしません。',
    backToTools: 'ツールに戻る',
    qualityLabel: '品質：',
    convertAll: 'すべて変換',
    retry: '再試行',
    downloadAllZip: 'すべてダウンロード (.zip)',
    dropImagesHere: 'ここに画像をドロップ',
    supportedFormats: 'JPG、PNG、WEBP、GIF、SVG 対応',
    selectFiles: 'ファイルを選択',
    converterWeightSaved: '節約されたサイズ',
    converterWeightIncreased: '増加したサイズ',
    converterPercentageSaved: '節約',
    converterPercentageIncreased: '増加',
    converterSavedBadge: '節約 {pct}%',
    converterIncreasedBadge: '増加 {pct}%',
    converterBefore: '変換前',
    converterAfter: '変換後',
    converterWeightReduced: 'のサイズ',
    converterTryDemo: 'デモ画像で試す',
    converterDropImages: '画像を上部にドラッグ＆ドロップ：変換は自動で行われ、即座に節約されたサイズが表示されます。',
    converterFailed: '失敗',
    converterPending: '待機中',
    converterUnexpectedError: '処理中に予期しないエラーが発生しました。',
    converterCorruptedFile: '画像ファイルが破損しているか読み取れません。',
    converterArchiveError: 'アーカイブの作成中にエラーが発生しました。',
    converterDemoLoadError: 'デモ画像を読み込めませんでした。再試行するか、ご自身の画像をドロップしてください。',
    launch: '起動',
    searchPlaceholder: 'プロジェクトを検索...',
    startProject: 'プロジェクトを提案',
    submitProjectTitle: 'プロジェクトについて相談する',
    submitProjectDesc: '以下のフォームにご記入ください。すぐにご連絡いたします。',
    firstName: '名',
    lastName: '姓',
    email: 'メールアドレス',
    projectDetails: 'プロジェクトの詳細',
    attachFile: 'ファイルを添付',
    filePlaceholder: 'ファイルをドラッグまたはクリック',
    send: '送信する',
    cancel: 'キャンセル',
    successMessage: 'リクエストは正常に送信されました！',
    memeTopText: '上部のテキスト',
    memeBottomText: '下部のテキスト',
    memeUpload: '画像をアップロード',
    memeDownload: 'ミームをダウンロード',
    memeFontSize: 'フォントサイズ',
    memeTextColor: 'テキストの色',
    memeStrokeColor: 'アウトラインの色',
    memeDragDrop: 'ここに画像をドラッグ＆ドロップ',
    memeExampleTop: 'コードが動いた',
    memeExampleBottom: 'しかも一発で',
    // Footer
    footerQuote: 'モダンなWeb体験を作り出すフルスタック開発者。',
    sections: 'セクション',
    pages: 'ページとツール',
    connect: 'SNS',
    builtWith: '使用技術',
    hostedOn: 'ホスティング',
    rights: '無断複写・転載を禁じます。',
    allProjects: 'すべてのプロジェクト',
    // Students
    students: '学生',
    studentsTitle: '学生の実績',
    studentsSubtitle: 'トレーニング中に学生が制作したポートフォリオ。',
    studentsCount: '件の注目ポートフォリオ',
    studentsHighlight: 'トレーニング中に制作',
    // SEO
    seoHomeTitle: "Cherif Diouf | フルスタック開発者、デジタル化専門家 & 講師 – ダカール",
    seoHomeDesc: "El Hadji Ahmadou Cherif Diouf のポートフォリオ。Laravel、React、Supabase & DevOps のエキスパート。セネガルでのWeb/モバイル開発、企業デジタル化とプロンプトエンジニアリング。",
    seoGalleryTitle: "技術イベント & ギャラリー | Cherif Diouf – ダカール",
    seoGalleryDesc: "セネガルのカンファレンス、ハッカソン、卒業式のハイライト。フルスタック開発者 Cherif Diouf。",
    seoToolsTitle: "無料開発者ツール | Cherif Diouf",
    seoToolsDesc: "無料オープンソースツール：GitHub READMEジェネレーター、WebP画像コンバーター、ミームクリエーター。セネガルのフルスタック開発者 Cherif Diouf 提供。",
    seoStudentsTitle: "学生の実績展示 | Cherif Diouf 講師",
    seoStudentsDesc: "ダカール（ESTM、ISM、ESCOA）でのWeb開発研修中に Cherif Diouf の学生が制作したポートフォリオ。",
    // Profile switcher
    profileSoftware: 'ソフトウェア工学',
    profileNetwork: 'ネットワーク管理',
    profileTeaching: '教育',
    // Hero entry CTAs (issue #22)
    heroCtaDev: '開発者を探している',
    heroCtaDigitalization: 'デジタル化専門家を探している',
    heroCtaTrainer: '講師を探している',
    viewProjects: '作品を見る →',
    contactCta: '話しましょう',
    discussProject: 'プロジェクトについて相談する',
    publications: '論文',
    // Profile-specific bios (issue #22)
    bioShortSoftware: 'フルスタック開発者。モダンで高性能、スケーラブルなWebアプリを構築 (React, Laravel, Supabase, DevOps)。',
    bioShortNetwork: 'デジタル化推進の専門家 & システム管理者。インフラ、ネットワーク、業務プロセスを最適化し、大幅な時間短縮を実現。',
    bioShortTeaching: '開発講師。プロジェクト実践と教育法で、学生とチームを技術的自立へ導く。',
  }
};
