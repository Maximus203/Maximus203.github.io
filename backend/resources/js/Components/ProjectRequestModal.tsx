...
const defaultLabels = {
    submitProjectTitle: "Proposer un projet",
    submitProjectDesc: "Vous avez une idée ? Parlons-en !",
    successMessage: "Votre projet a été soumis avec succès. Je vous recontacterai bientôt.",
    firstName: "Prénom",
    lastName: "Nom",
    email: "Email",
    projectDetails: "Détails du projet",
    attachFile: "Joindre un fichier (optionnel)",
    filePlaceholder: "Glissez un fichier ou cliquez pour parcourir",
    cancel: "Annuler",
    send: "Envoyer",
    errorTitle: "Erreur",
...
const ProjectRequestModal: React.FC<ProjectRequestModalProps> = ({ isOpen, onClose, labels = defaultLabels }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await axios.post('/api/projects/submit', data, {
            headers: {
        ...
        if (error.response && error.response.status === 422) {
            ...
            setErrors({ general: labels.genericError });
        }
    } finally {
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-800 border ${errors.firstName ? 'border-red-500' : 'border-gray-200 dark:border-slate-700'} focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900/30 outline-none transition-all text-sm text-gray-900 dark:text-white placeholder-gray-400`}
                        />
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-800 border ${errors.lastName ? 'border-red-500' : 'border-gray-200 dark:border-slate-700'} focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900/30 outline-none transition-all text-sm text-gray-900 dark:text-white placeholder-gray-400`}
                        />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-800 border ${errors.email ? 'border-red-500' : 'border-gray-200 dark:border-slate-700'} focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900/30 outline-none transition-all text-sm text-gray-900 dark:text-white placeholder-gray-400`}
                      />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        className={`w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-800 border ${errors.details ? 'border-red-500' : 'border-gray-200 dark:border-slate-700'} focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900/30 outline-none transition-all text-sm text-gray-900 dark:text-white placeholder-gray-400 resize-none`}
                      />
                      {errors.details && <p className="text-xs text-red-500 ml-1">{errors.details}</p>}
                    </div>
                    ...
                    <div className="space-y-1.5">
                       ...
                       <div
                         ...
                         className={`border-2 border-dashed ${errors.file ? 'border-red-500' : 'border-gray-300 dark:border-slate-700'} hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 rounded-xl p-6 transition-all cursor-pointer text-center group`}
                       >
                         <input
                           type="file"
                           ref={fileInputRef}
                       ...
                       {errors.file && <p className="text-xs text-red-500 ml-1">{errors.file}</p>}
                    </div>
